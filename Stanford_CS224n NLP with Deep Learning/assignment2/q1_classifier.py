...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        #first step is to initial data
        W = tf.Variable(initial_value = tf.zeros(shape = (self.config.n_features, self.config.n_classes)), dtype = tf.float32)
        b = tf.Variable(initial_value = tf.zeros(shape = self.config.n_classes), dtype = tf.float32)
